...
interface TasksTodoViewProps {
    ...
    canEdit: boolean;
    onStatusChange: (taskNanoid: string, oldStatus: TaskStatus, newStatus: TaskStatus) => void;
...
export function TasksTodoView({
    ...
    const toggleComplete = (task: Task) => {
        if (task.status === 'done') {
            ...
            onStatusChange(task.nanoid, 'done', 'now');
            // Also update is_completed to false
            onTaskUpdate(task.nanoid, task.status, { is_completed: false });
        } else {
            // Mark as done and update status
            onStatusChange(task.nanoid, task.status, 'done');
            // Also update is_completed to true
            onTaskUpdate(task.nanoid, task.status, { is_completed: true });
        }
    };
