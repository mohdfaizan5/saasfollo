...
function RevealedQuestion({
  ...
  return (
    <AnimatePresence initial={false}>
      {visible ? (
        <motion.div ref={ref} {...revealMotion}>
          {children}
        </motion.div>
      ) : null}
    </AnimatePresence>
...
export function BranchAQuestions({
  ...
  const showSecond = hasValue(answers.stopping_from_shipping);
  const showThird = hasValue(answers.defined_first_version);
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <RevealedQuestion visible={showSecond}>
        <h3 className="mb-3 text-lg font-semibold text-foreground">
      ...
      <RevealedQuestion visible={showThird}>
...
export function BranchBQuestions({
  ...
  const showSecond = hasValue(answers.biggest_bottleneck);
  const showThird = hasValue(answers.version_goals_or_backlog);
      ...
      <motion.div {...revealMotion}>
        ...
        <OnboardingRadioGroup
          ...
          options={[
            ...
            { value: 'marketing_anxiety', label: 'Marketing anxiety', description: 'Worried about launch and growth', icon: <Megaphone weight="duotone" size={22} /> },
          ]}
        />
      </motion.div>
...
export function BranchCQuestions({
  ...
  const selectedChannels = answers.getting_users_how ?? [];
  const showSecond = selectedChannels.length > 0;
  const showThird = hasValue(answers.whats_happening);
    ...
    <div className="flex flex-col gap-8">
      <motion.div {...revealMotion}>
        ...
        <OnboardingCheckboxGroup
          ...
          onOptionSelect={onOptionSelect}
          options={[
            { value: 'content_seo', label: 'Content / SEO', description: 'Organic traffic through content and search', icon: <MagnifyingGlass weight="duotone" size={22} /> },
            { value: 'cold_outreach', label: 'Cold outreach', description: 'Direct DMs or email to potential users', icon: <Envelope weight="duotone" size={22} /> },
            { value: 'communities', label: 'Communities (Reddit, Slack, Discord, etc.)', description: 'Trying to get attention where your audience already hangs out', icon: <ChatDots weight="duotone" size={22} /> },
            { value: 'referrals_network', label: 'Referrals / personal network', description: 'Friends, warm intros, and word of mouth', icon: <Users weight="duotone" size={22} /> },
            { value: 'paid_ads', label: 'Paid ads', description: 'Running acquisition through paid channels', icon: <CurrencyCircleDollar weight="duotone" size={22} /> },
            { value: 'not_consistent', label: "I'm not doing this consistently yet", description: 'You are trying things, but there is no steady rhythm yet', icon: <ArrowsClockwise weight="duotone" size={22} /> },
          ]}
        />
      </motion.div>
...
export function BranchDQuestions({
  ...
  const showSecond = hasValue(answers.mrr_range);
  const showThird = hasValue(answers.growth_bottleneck);
...
export function BranchEQuestions({
  ...
  const showSecond = hasValue(answers.energy_drain);
